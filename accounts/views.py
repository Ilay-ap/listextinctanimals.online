...
def register_view(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.success(
                request,
                _("Welcome, %(username)s! Your account was created successfully.")
                % {"username": username},
            )
            return redirect("mammals:index")
            ...
            messages.error(
                request, _("Error creating account: %(error)s") % {"error": str(e)}
            )
            return render(request, "accounts/register.html")
...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.success(
                request, _("Welcome, %(username)s!") % {"username": user.username}
            )
    ...
    if request.method == "POST":
        ...
        if new_password:
            ...
            if len(new_password) < 6:
                messages.error(
                    request, _("New password must be at least 6 characters.")
                )
                return render(request, "accounts/edit_profile.html")
        ...
        try:
            ...
            messages.error(
                request, _("Error updating profile: %(error)s") % {"error": str(e)}
            )
